...
class FaceMakeup(object):
    def __init__(self, app_id, app_key):
        ...
        """
        self.request_url = "https://api.ai.qq.com/fcgi-bin/ptu/ptu_facedecoration"
        self.me_data = {
            "app_id": app_id,
            "app_key": app_key
        }
    ...
    def start(self, decoration, img_path):
        """
        :param decoration: 变妆编码 （int）
        :param img_path: 图片路径  (Str)
        :return: JSON数据
        """
        request_AI_FaceMakeup = {
        ...
        request_AI_FaceMakeup["sign"] = self.calculate_sign(request_AI_FaceMakeup, self.me_data["app_key"])
        request_data = sorted(request_AI_FaceMakeup.items())
