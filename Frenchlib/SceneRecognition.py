...
class SceneRecognition(object):
    def __init__(self, app_id, app_key):
        ...
        """
        self.request_url = "https://api.ai.qq.com/fcgi-bin/vision/vision_scener"
        self.me_data = {
            "app_id": app_id,
            "app_key": app_key
        }
    ...
    def start(self, img_path, to_pk=5):
        """
        :param img_path: 图片路径(Str)
        :param to_pk: 返回结果个数（已按置信度倒排） 默认5（int）
        :return:
        """
        request_AI_SceneRecognition = {
        ...
        request_AI_SceneRecognition["sign"] = self.calculate_sign(request_AI_SceneRecognition,
                                                                  self.me_data["app_key"])
        request_data = sorted(request_AI_SceneRecognition.items())
