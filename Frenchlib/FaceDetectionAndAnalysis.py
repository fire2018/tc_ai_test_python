...
class FaceDetectionAndAnalysis(object):
    def __init__(self, app_id, app_key):
        ...
        """
        self.request_url = "https://api.ai.qq.com/fcgi-bin/face/face_detectface"
        self.me_data = {
            "app_id": app_id,
            "app_key": app_key
        }
    ...
    def start(self, img_path, mode=1):
        ...
        :param mode: 检测模式，0-正常，1-大脸模式（默认1）  (int)
        :return: JSON数据
        """
        request_AI_FaceDetectionAndAnalysis = {
        ...
        request_AI_FaceDetectionAndAnalysis["sign"] = self.calculate_sign(request_AI_FaceDetectionAndAnalysis,
                                                                          self.me_data["app_key"])
        request_data = sorted(request_AI_FaceDetectionAndAnalysis.items())
