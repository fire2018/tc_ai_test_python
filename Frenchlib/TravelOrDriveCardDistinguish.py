...
class TravelOrDriveCardDistinguish(object):
    def __init__(self, app_id, app_key):
        ...
        """
        self.request_url = "https://api.ai.qq.com/fcgi-bin/ocr/ocr_driverlicenseocr"
        self.me_data = {
            "app_id": app_id,
            "app_key": app_key,
        }
    ...
    def start(self, img_path, int_type):
        """

        :param img_path: 图片路径  (Str)
        :param int_type: 图片类型,0为行驶证，1位驾驶证 （int）
        :return: json数据
        """
        request_AI_TravelOrDriveCardDistinguish = {
        ...
        request_AI_TravelOrDriveCardDistinguish["sign"] = self.calculate_sign(request_AI_TravelOrDriveCardDistinguish,
                                                                              self.me_data["app_key"])
        req_data = sorted(request_AI_TravelOrDriveCardDistinguish.items())
