...
class FaceFusion(object):
    def __init__(self, app_id, app_key):
        ...
        """
        self.request_url = "https://api.ai.qq.com/fcgi-bin/ptu/ptu_facemerge"
        self.me_data = {
            "app_id": app_id,
            "app_key": app_key
        }
    ...
    def start(self, model, img_path):
        """

        :param model: 素材模板编码  (int)
        :param img_path: 图片路径  (Str)
        :return: JSON数据
        """
        request_AI_FaceFusion = {
        ...
        request_AI_FaceFusion["sign"] = self.calculate_sign(request_AI_FaceFusion, self.me_data["app_key"])
        request_data = sorted(request_AI_FaceFusion.items())
